...
static struct gpio_event_direct_entry shooter_u_gpio_switch[] = {
	{
		.gpio = SHOOTER_U_GPIO_SW_LCM_3D,
		.code = SW_CAM,
		.not_wakeup_src = true,
	},
};

static void shooter_u_setup_switch_gpio(void)
{
	uint32_t inputs_gpio_table[] = {
		GPIO_CFG(SHOOTER_U_GPIO_SW_LCM_3D, 0, GPIO_CFG_INPUT,
			GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
		GPIO_CFG(SHOOTER_U_GPIO_SW_LCM_2D, 0, GPIO_CFG_INPUT,
			GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
	};
...
static struct gpio_event_switch_info shooter_u_keypad_switch_info = {
...
# endif
	...
	.setup_switch_gpio = shooter_u_setup_switch_gpio,
};
